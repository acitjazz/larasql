...
export default defineComponent({
  ...
  created() {
    this.rows = this.modelValue ??  [{ title: null, url:null, type: '_self' }];
  },
  methods:{
    addRow(){
        this.$refs.tree.add({ title: null, url: null, type: '_self' });
        this.save();
    },
    removeRow(stat){
  ...
  <div @mouseleave="save" id="input-buttons">
    <Draggable v-model="rows" ref="tree" virtualization
      ...
      <template #default="{ node, stat }">
        <span style="display:none" v-if="stat.children.length" @click="stat.open = !stat.open">
          {{ stat.open ? "-" : "+" }}
        </span>
