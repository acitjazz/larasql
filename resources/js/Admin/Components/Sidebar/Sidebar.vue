            ...
            <ul class="md:flex-col md:min-w-full flex flex-col list-none" :class="section.class">
              <li class="items-center" v-for="(menu, index) in section.menus" :key="index">
                ...
                <Link
                  ...
                  :method="menu.method"
                  :as="menu.as"
                  :class="[{'bg-gray-100 border-l-2 border-primary': currentPath === menu.link},menu.class]"
                >
                  <i class="fas text-[#777] mr-2 text-sm" :class="menu.icon"></i>
                  {{menu.title}}
                </Link>
...
export default {
  ...
  mounted: function () {
    // if(!localStorage.getItem('hasTour')){
  ...
  methods: {
    ...
    fetchMenus() {
        axios.get(route('menu')).then(response => {
    ...
    onFinish(){
        localStorage.setItem('hasTour',true);
    },
    onSkip(){
        console.log('SKIP')
        localStorage.setItem('hasTour',true);
